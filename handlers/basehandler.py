...
class BaseHandler:
    ...
    def __init__(self, payload, workflow_json = None):
        ...
        self.aws_key_prefix = str(self.get_value(
            "aws_key_prefix",
            ""
        )
                              )
    ...
    def get_url_content(self, url):
        ...
        if existing_file:
            print("get_url_content returning existing file")
            return os.path.basename(existing_file)
        else:
            print("get_url_content downloading file")
    ...
    def get_result(self, job_id):
        ...
        print(f"custom_output_dir is {custom_output_dir}")
                ...
                for image in outputs[item]["images"]:
                    if image['type'] and image['type'] is "output":
                        ...
                        if os.path.islink(original_path):
                            print(f"it is a link")
                            shutil.copyfile(os.path.realpath(original_path), new_path)
                        else:
                            print(f"rename orig to new")
                            os.rename(original_path, new_path)
                            os.symlink(new_path, original_path)
                        key = f"{self.aws_key_prefix}" # {self.request_id}/{image['filename']}
